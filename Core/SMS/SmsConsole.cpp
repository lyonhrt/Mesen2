...
// to the internal SMS HdPackBuilderOptions defined in HdPackBuilderSms.h
struct SmsHdPackInteropOptions {
    char* SaveFolder;              // [MarshalAs(UnmanagedType.LPUTF8Str)] string
    ScaleFilterType FilterType;    // not used by SMS builder
    uint32_t Scale;
    uint32_t ChrRamBankSize;       // mapped to VramBankSize for SMS
    bool UseLargeSprites;          // not used by SMS builder
...
void SmsConsole::ProcessNotification(ConsoleNotificationType type, void* parameter)
{
    if(type == ConsoleNotificationType::ExecuteShortcut) {
        ...
        switch(params->Shortcut) {
            default: break;
            case EmulatorShortcut::StartRecordSmsHdPack: {
                // Convert UI interop options to internal SMS options to avoid struct layout mismatch
                SmsHdPackInteropOptions* interop = (SmsHdPackInteropOptions*)params->ParamPtr;
